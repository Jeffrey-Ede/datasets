...
class vaegan(object):
    ...
    def build_model_vaegan(self):
        ...
        self.sigma = tf.sqrt(tf.exp(self.z_sigm))
        self.z_x = tf.add(self.z_mean, tf.sqrt(tf.exp(self.z_sigm))*self.ep) #tf.add(self.z_mean, self.z_sigm*self.ep)# 
        self.x_tilde = self.generate(self.z_x, reuse=False)

        #KL loss
        self.dist_loss = tf.reduce_mean( self.z_mean**2 + tf.exp(self.z_sigm) - self.z_sigm - 1 )
        self.kl_loss = tf.sqrt(tf.exp(self.z_sigm[0,0]))#self.KL_loss(self.z_mean, self.z_sigm) / (self.latent_dim*self.batch_size)
    ...
    def test(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(num_batches):
                ...
                [latent, error] = sess.run([self.z_mean, self.sigma], feed_dict={self.images: next_x_images, self.beta1_ph: np.float32(0.9)})
