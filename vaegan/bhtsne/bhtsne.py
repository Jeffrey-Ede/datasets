...
def run_bh_tsne(data, sigma, no_dims=2, perplexity=50, theta=0.5, randseed=-1, verbose=False, initial_dims=50, use_pca=True, max_iter=1000):
    '''
    ...
    if _is_filelike_object(data):
        data = load_data(data)
    if _is_filelike_object(sigma):
        sigma = load_data(sigma)

    data = np.concatenate(tuple([data, sigma]), axis=0)
